...
function load(keyWord, pageIndex, pageSize) {
    $.ajax({
        url: "/Admin/DanhMuc/GetPagingLoaiCV",
        data: {
            keyWord: keyWord,
            pageIndex: pageIndex,
            pageSize: pageSize
        ...
        success: function (response) {
            ...
            $.each(response.data, function (index, value) {
                str += "<tr>";
                str += "<td>" + value.MaLoaiCV + "</td>";
                str += "<td>" + value.TenLoaiCV + "</td>";
                str += '<td class="d-flex"><a class="btn btn-warning" data-edit=' + value.MaLoaiCV + '>Sửa</a>';
                str += '<a class="btn btn-danger ml-1" href="#" data-delete=' + value.MaLoaiCV + '>Xóa</a>';
                str += "</tr>";
...
btnCreate.on("click", function (event) {
    ...
    let name = prompt("Nhập tên loại công việc:");
    if (name) {
        $.ajax({
            url: "/Admin/DanhMuc/CreateLoaiCV",
            type: "POST",
            data: { TenLoaiCV: name },
            dataType: "json",
            success: (result) => {
...
$("body").on("click", "#datatablesSimple a.btn.btn-danger", function (event) {
    ...
    if (confirm("Bạn có muốn xóa loại công việc có mã " + itemDelete + " này không?")) {
        $.ajax({
            url: "/Admin/DanhMuc/DeleteLoaiCV",
            type: "POST",
            data: { MaLoaiCV: itemDelete },
            dataType: "json",
            success: (result) => {
                location.reload();
            }
...
$("body").on("click", "#datatablesSimple a.btn.btn-warning", function (event) {
    ...
    let value = prompt("Nhập tên loại công việc mới:");

    if (value) {
        $.ajax({
            url: "/Admin/DanhMuc/EditLoaiCV",
            type: "POST",
            data: {
                MaLoaiCV: itemEdit,
                TenLoaiCV: value
