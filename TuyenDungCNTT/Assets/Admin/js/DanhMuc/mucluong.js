...
function load(keyWord, pageIndex, pageSize) {
    $.ajax({
        url: "/Admin/DanhMuc/GetPagingMucLuong",
        data: {
            keyWord: keyWord,
            pageIndex: pageIndex,
            pageSize: pageSize
        ...
        success: function (response) {
            ...
            $.each(response.data, function (index, value) {
                str += "<tr>";
                str += "<td>" + value.MaMucLuong + "</td>";
                str += "<td>" + value.TenMucLuong + "</td>";
                str += '<td class="d-flex"><a class="btn btn-warning" data-edit=' + value.MaMucLuong + '>Sửa</a>';
                str += '<a class="btn btn-danger ml-1" href="#" data-delete=' + value.MaMucLuong + '>Xóa</a>';
                str += "</tr>";
...
btnCreate.on("click", function (event) {
    ...
    if (name) {
        $.ajax({
            url: "/Admin/DanhMuc/CreateMucLuong",
            type: "POST",
            data: { TenMucLuong: name },
            dataType: "json",
            success: (result) => {
                location.reload();
            }
...
$("body").on("click", "#datatablesSimple a.btn.btn-danger", function (event) {
    ...
    if (confirm("Bạn có muốn xóa mức lương có mã " + itemDelete + " này không?")) {
        $.ajax({
            url: "/Admin/DanhMuc/DeleteMucLuong",
            type: "POST",
            data: { MaML: itemDelete },
            dataType: "json",
            success: (result) => {
                location.reload();
            }
...
$("body").on("click", "#datatablesSimple a.btn.btn-warning", function (event) {
    ...
    let value = prompt("Nhập tên mức lương mới:");

    if (value) {
        $.ajax({
            url: "/Admin/DanhMuc/EditMucLuong",
            type: "POST",
            data: {
                MaMucLuong: itemEdit,
                TenMucLuong: value
